...
export default function Comments() {
  ...
  return React.createElement('div', null,
    ...
    : React.createElement('div', { className: 'space-y-3' }, comments.map((c: any) =>
        React.createElement('div', { key: c.id, className: 'card p-4' }, React.createElement('input', { type: 'checkbox', checked: selected.has(c.id), onChange: () => toggle(c.id), className: 'float-right w-4 h-4 rounded border-gray-300 text-primary-600' }),
          React.createElement('div', { className: 'flex items-start justify-between' },
            React.createElement('div', { className: 'flex-1' },
              React.createElement('div', { className: 'flex items-center gap-2 mb-1' }, React.createElement('span', { className: 'font-medium text-sm' }, c.author), c.email && React.createElement('span', { className: 'text-xs text-gray-400' }, c.email), React.createElement('span', { className: `px-1.5 py-0.5 text-xs rounded-full ${c.status === 'approved' ? 'bg-green-100 text-green-700' : c.status === 'spam' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}` }, c.status)),
            ...
            React.createElement('div', { className: 'flex items-center gap-1 ml-4' },

...
export default function Webhooks() {
  ...
  const save = async () => {
    ...
    try {
      if (editing.id) {
        ...
        setOneTimeSecret(r.data.secret);
  ...
  return React.createElement('div', null,
    ...
    React.createElement('div', { className: 'flex items-center justify-between mb-2' },
      ...
      React.createElement('button', { onClick: () => { setEditing({ id: null, name: '', url: '', events: ['post_published'] }); setOneTimeSecret(null); }, className: 'btn-primary text-sm' },
        React.createElement(Plus, { size: 15 }), t('new webhook', getLang())))
    ),
    React.createElement('p', { className: 'text-sm text-gray-500 dark:text-gray-400 mb-4' }, t('webhooks description', getLang())),
    React.createElement('div', { className: 'text-xs text-gray-400 dark:text-gray-500 mb-6 flex items-start gap-1.5' },
      React.createElement(Info, { size: 13, className: 'mt-0.5 shrink-0' }),
      t('payload format note', getLang())),

    // One-time secret banner (after create / reset — shown exactly once)
    oneTimeSecret && React.createElement('div', { className: 'card p-4 mb-6 border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/20' },
    ...
    loading
      ...
      : webhooks.length === 0
        ? React.createElement('div', { className: 'card p-10 text-center' },
            React.createElement('p', { className: 'text-sm text-gray-400' }, t('no webhooks yet', getLang())))
        : React.createElement('div', { className: 'space-y-4' },
            webhooks.map(w =>
              React.createElement('div', { key: w.id, className: 'card p-5' },
                React.createElement('div', { className: 'flex items-start gap-3' },
                  ...
                  React.createElement('div', { className: 'flex-1 min-w-0' },
                    React.createElement('div', { className: 'flex items-center gap-2 flex-wrap' },
                      ...
                      // Delivery status
                      w.lastStatus === null
                        ? React.createElement('span', { className: 'text-[10px] px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400' }, t('never sent', getLang()))
                        : w.lastStatus >= 200 && w.lastStatus < 300
                          ? React.createElement('span', { className: 'text-[10px] px-2 py-0.5 rounded-full bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300 flex items-center gap-1' }, React.createElement(CheckCircle2, { size: 10 }), t('success', getLang()) + ' · ' + w.lastStatus)
                          : React.createElement('span', { className: 'text-[10px] px-2 py-0.5 rounded-full bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300 flex items-center gap-1' }, React.createElement(XCircle, { size: 10 }), t('failed', getLang()) + ' · ' + w.lastStatus),
                    ...
                    React.createElement('div', { className: 'flex flex-wrap gap-1.5 mt-2' },
                      w.events.map((ev, i) => React.createElement('span', { key: ev, className: 'text-[10px] px-2 py-0.5 rounded-full font-medium ' + chipColor(i) }, t('event ' + ev, getLang())))),
                    // Last delivery info
                    w.lastError && React.createElement('p', { className: 'text-xs text-red-500 dark:text-red-400 mt-2' }, t('last error', getLang()) + ': ' + w.lastError),
                    w.lastSentAt && React.createElement('p', { className: 'text-[11px] text-gray-400 mt-1' }, t('last delivery', getLang()) + ': ' + new Date(w.lastSentAt).toLocaleString()),
                  ),
                  // Actions
                  React.createElement('div', { className: 'flex flex-col gap-1.5 shrink-0' },
                    React.createElement('button', { onClick: () => test(w), title: t('test webhook', getLang()), className: 'btn-secondary text-xs px-2.5 py-1' }, React.createElement(Send, { size: 12 }), t('test webhook', getLang())),
                    React.createElement('div', { className: 'flex gap-1.5' },
                      React.createElement('button', { onClick: () => { setEditing({ id: w.id, name: w.name, url: w.url, events: w.events }); setOneTimeSecret(null); }, title: t('edit', getLang()), className: 'btn-secondary text-xs px-2.5 py-1' }, React.createElement(Pencil, { size: 12 })),
    ...
    // Create / edit modal
    editing && React.createElement('div', { className: 'fixed inset-0 z-[60] bg-black/40 flex items-center justify-center p-4', onClick: (e: React.MouseEvent) => { if (e.target === e.currentTarget) { setEditing(null); setOneTimeSecret(null); } } },
      React.createElement('div', { className: 'card w-full max-w-lg p-6' },
        ...
        React.createElement('input', {
          value: editing.url,
          onChange: (e: React.ChangeEvent<HTMLInputElement>) => setEditing({ ...editing, url: e.target.value }),
        ...
        React.createElement('div', { className: 'grid grid-cols-2 gap-1.5 mb-6 max-h-56 overflow-y-auto pr-1' },
          EVENTS.map((ev, i) =>
            React.createElement('label', { key: ev, className: 'flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg px-2 py-1.5' },
              React.createElement('input', {
                type: 'checkbox',
                checked: editing.events.includes(ev),
                onChange: (e: React.ChangeEvent<HTMLInputElement>) => setEditing({
                  ...editing,
                  events: e.target.checked ? [...editing.events, ev] : editing.events.filter(x => x !== ev),
                }),
                className: 'accent-primary-600',
              }),
              React.createElement('span', { className: 'inline-block w-2 h-2 rounded-full mr-1 ' + chipColor(i) }),
              t('event ' + ev, getLang()))),
        React.createElement('div', { className: 'flex justify-end gap-2' },
          React.createElement('button', { onClick: () => { setEditing(null); setOneTimeSecret(null); }, className: 'btn-secondary text-sm' }, t('cancel', getLang())),

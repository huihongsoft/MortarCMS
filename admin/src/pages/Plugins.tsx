...
import { Puzzle, Zap, Filter, Link2, PlusCircle, RefreshCw, ToggleLeft, ToggleRight, Package, Store, Download, Trash2 } from 'lucide-react';
...
export default function Plugins() {
  ...
  return React.createElement('div', null,
    ...
    React.createElement('p', { className: 'text-sm text-gray-500 mb-6' }, t('wordpress-style plugin system with a plugin market.', getLang())),
    ...
    tab === 'installed' && React.createElement('div', { className: 'card p-5 mb-6' },
      ...
      plugins.length === 0
        ? React.createElement('p', { className: 'text-xs text-gray-400' }, t('no plugins installed. add a folder with plugin.json + index.ts to server/plugins.', getLang()))
        : React.createElement('div', { className: 'space-y-3' },
            plugins.map((p: any) =>
              React.createElement('div', { key: p.name, className: 'flex items-start gap-3 p-3 bg-gray-50 rounded-lg' },

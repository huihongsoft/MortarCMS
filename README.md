...
> Supports Linux / macOS. SQLite is the supported (and recommended) database. MySQL / PostgreSQL can be connected via `DATABASE_URL` but are **experimental** — see [Database](#-database).
...
1. Choose a database — **SQLite** (default, zero-config). MySQL/MariaDB and
   PostgreSQL are offered but **experimental** (see below).
...
Mortar is designed and supported as a **single-instance deployment on SQLite**.
The adapter also accepts MySQL/PostgreSQL through `DATABASE_URL`, but those
drivers are **experimental and not recommended for production**: they run over
a single-connection worker thread that can block the request loop on slow
queries, and they have received far less testing. Use them for local
evaluation only.

| Engine | Status | Notes |
|--------|--------|-------|
| SQLite | ✅ Supported (recommended) | Single file at `server/data/mortar.db`; WAL, `busy_timeout` and foreign-key enforcement enabled |
| MySQL / MariaDB | ⚠️ Experimental | Not recommended for production; auto-creates the database (utf8mb4) |
| PostgreSQL | ⚠️ Experimental | Not recommended for production |

**Scaling note.** SQLite, on-disk uploads and the in-process cache/scheduler
mean Mortar runs as a **single instance**. Running multiple replicas against
one SQLite file, or behind a load balancer without shared storage, is **not
supported** — scheduled tasks would run on every replica and uploads/caches
would diverge.

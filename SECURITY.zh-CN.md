...
Mortar 的支持场景是**基于 SQLite 的单实例部署**。MySQL 与 PostgreSQL 属于
实验性，不推荐用于生产。请勿让多个副本共用同一 SQLite 文件，或在无共享存储
的情况下置于负载均衡之后：调度器、本地磁盘上传与进程内缓存都假定单进程运行
（详见 README → 数据库）。运行多个实例会导致定时任务重复执行，上传文件与缓存
不一致。
